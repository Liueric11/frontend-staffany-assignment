...
export const getShiftsByStartDate = async (startDate: string) => {
  const api = getAxiosInstance()
  const { data } = await api.get(`/weeks/${startDate}`);
  return data;
};
...
export const publishWeek = async (startDate: string) => {
  const api = getAxiosInstance()
  const { data } = await api.patch(`/weeks/${startDate}`);
  return data;
};